...
def forecast_models(df_model, target_col='Total final consumption (PJ)', split_year=2015, models_to_run=None, plot=True, future_years=None): 
    ...
    for name, model in models.items():
        ...
        if plot:
            ...
            ax.plot(df_pred['Year'], df_pred['Test Prediction'], color= 'darkblue', label='Test Prediction')
...
if country == 'Ireland':
    ...
    if isinstance(Si, dict):
        ...
        s1_values = getattr(Si, 'S1', [0]*len(features_list))
    s1_map = dict(zip(features_list, s1_values))
    table_rows = []
    ...
    st.caption(f'Highly regulated market, No sensitivity analysis available since features has no impact on energy price')
